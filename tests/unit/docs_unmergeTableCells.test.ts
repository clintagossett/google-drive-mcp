...
describe('docs_unmergeTableCells - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should validate correct parameters for 2x2 unmerge', () => {
      const validInput = {
      ...
      const result = DocsUnmergeTableCellsSchema.safeParse(validInput);
    ...
    it('should reject missing documentId', () => {
      ...
      const result = DocsUnmergeTableCellsSchema.safeParse(invalidInput);
  ...
  describe('API Request Formation', () => {
    it('should form correct batchUpdate request structure', () => {
      ...
      expect(expectedRequest.requestBody.requests[0].unmergeTableCells.tableRange.rowSpan).toBe(2);
      expect(expectedRequest.requestBody.requests[0].unmergeTableCells.tableRange.columnSpan).toBe(2);
    });
  });
});
