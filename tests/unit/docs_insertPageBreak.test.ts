...
const DocsInsertPageBreakSchema = z.object({
  ...
  index: z.number().int().min(1, "Index must be >= 1")
});

describe('docs_insertPageBreak - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should validate correct parameters', () => {
      ...
      const result = DocsInsertPageBreakSchema.safeParse(validInput);
    ...
    it('should reject missing documentId', () => {
      ...
      const result = DocsInsertPageBreakSchema.safeParse(invalidInput);
    ...
    it('should reject index less than 1', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.errors[0].message).toBe('Index must be >= 1');
      }
    });
  ...
  describe('API Request Formation', () => {
    it('should form correct batchUpdate request structure', () => {
      ...
      expect(expectedRequest.requestBody.requests[0].insertPageBreak.location.index).toBe(input.index);
    });
    ...
    it('should handle large index values', () => {
      ...
      const result = DocsInsertPageBreakSchema.safeParse(input);
      expect(result.success).toBe(true);
      if (result.success) {
