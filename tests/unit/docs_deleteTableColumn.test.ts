...
describe('docs_deleteTableColumn - Unit Tests', () => {
  describe('Schema Validation', () => {
    it('should validate correct parameters', () => {
      ...
      const result = DocsDeleteTableColumnSchema.safeParse(validInput);
    ...
    it('should reject missing documentId', () => {
      ...
      const result = DocsDeleteTableColumnSchema.safeParse(invalidInput);
  ...
  describe('API Request Formation', () => {
    it('should form correct batchUpdate request structure', () => {
      ...
      expect(expectedRequest.requestBody.requests[0].deleteTableColumn.tableCellLocation.tableStartLocation.index).toBe(input.tableStartIndex);
      expect(expectedRequest.requestBody.requests[0].deleteTableColumn.tableCellLocation.rowIndex).toBe(input.rowIndex);
      expect(expectedRequest.requestBody.requests[0].deleteTableColumn.tableCellLocation.columnIndex).toBe(input.columnIndex);
    });
  });
});
